...
- **Skill**: a reusable capability with declared permissions and future signature support.
- **Approval**: a recorded human or non-interactive decision for a risky action.
- **Adapter**: a bridge between RunWitness and the thing doing work, starting with local shell commands.
...
apps/
  ...
  web/        Future operator cockpit.
  desktop/    Future desktop shell.
packages/
  adapters/   Bridges for shells and future agents.
  core/       Run types, ids, event ledger, and orchestration.
  policy/     Risk classification and approvals.
  receipts/   Receipt and proof-bundle exporters.
  sandbox/    Workspace snapshots and file diffs.
  skills/     Skill manifest foundation.
...
RunWitness starts with observation and receipt generation. The first policy engine can classify risky commands and record explicit approvals. Later phases should strengthen this into enforceable sandboxing, signed skills, secret brokering, network allowlists, and multi-user authorization.
